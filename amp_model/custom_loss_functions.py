...
# first check if the avg weights file exists
weight_file_name = "jr_magn_median_2011_2015_.npy"
file_path = pathlib.Path(weight_file_name)
if file_path.exists():
    avg_weights = np.load(weight_file_name)
else:
    file_path = pathlib.Path.cwd().joinpath("amp_model",\
                             weight_file_name).as_posix()
    avg_weights = np.load(file_path)
